...
(<\>) :: (Parser Char) -> (Parser Char) -> (Parser Char)
a <\> b = a <+> Ignore (optional whitespaceAndLinebreaks) <+> b
infixl 6 <\>

(<?>) :: (Parser Char) -> (Parser Char) -> (Parser Char)
a <?> b = a <+> optional (Ignore whitespace) <+> b
infixl 6 <?>
...
parse (Or a b) input = case ((parse a input),(parse b input)) of
    (a@(Parsing na va ra ina), b@(Parsing nb vb rb inb)) -> if length ina > length inb then b else a
    (a@(Parsing _ _ _ _), Fail) -> a
...
optional parser = parser <|> Empty
whitespaceAndLinebreaks = oneOrMore (oneOf ['\t', ' ', '\n', '\r'])
